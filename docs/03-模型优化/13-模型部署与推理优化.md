...
**背景问题：** 标准注意力需要将完整的 S&times;S 注意力矩阵（S 为序列长度）写入 HBM，再进行 softmax，HBM 访问次数为 O(S²)，成为瓶颈。
...
- **通信量**：每层 2 次 AllReduce，通信量 = 2 &times; hidden_size &times; seq_len &times; dtype_size
...
2. **KD Loss**：L = α &times; L_CE(y, student_logits) + (1-α) &times; KL(teacher_logits/T, student_logits/T)
...
- 计算量：1 &times; d &times; S（d 为 head_dim，S 为序列长度）
...
| Tiling 策略 | Q 分块 &times; KV 分块 | KV 分块，Q 不分 |
...
| 方案 | 月成本 | 适用场景 |
|------|--------|----------|
| 自建 (1&times;A100) | ~$800/月 | 高调用量，数据隐私要求高 |
| 自建 (1&times;4090) | ~$300/月 | 低延迟要求，本地部署 |
