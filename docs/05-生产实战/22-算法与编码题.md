...
| 维度 | 复杂度 |
|------|--------|
| 训练时间 | O(N &times; M &times; num_merges)，N=词数，M=平均词长 |
| 推理时间 | O(T &times; num_merges)，T=输入 token 数 |
| 空间 | O(V)，V=词表大小 |
...
| 维度 | 复杂度 |
|------|--------|
| 插入 | O(m &times; log(N) &times; D) 平均 |
| 搜索 | O(log(N) &times; D) 平均，远优于暴力 O(N&times;D) |
| 空间 | O(N &times; m &times; L)，L=平均层数 |
...
| 维度 | 复杂度 |
|------|--------|
| 摄入 | O(N &times; D)，N=文本块数，D=Embedding 维度 |
| 查询 | O(N &times; D + K &times; D) |
| 空间 | O(N &times; D) |
...
| 维度 | 复杂度 |
|------|--------|
| 索引 | O(N &times; L)，N=文档数，L=平均长度 |
| 查询 | O(N &times; Q)，Q=查询词数 |
| 空间 | O(N &times; V)，V=词表大小 |
...
| 维度 | 复杂度 |
|------|--------|
| 打分 | O(K &times; D)，K=候选数，D=模型维度 |
| 排序 | O(K log K) |
| 总查询 | 召回 O(log N) + Rerank O(K &times; D) |
...
| 总时间 | O(steps &times; LLM_latency) |
| 空间 | O(steps &times; context_length) |
...
| 添加（短期） | O(1) |
| 添加（长期） | O(1) |
| 搜索（长期） | O(N &times; Q)，N=记忆条数，Q=查询词数 |
...
| 维度 | 复杂度 |
|------|--------|
| 无缓存（生成 N tokens） | O(N² &times; D) |
| 有缓存（生成 N tokens） | O(N &times; D)（每次只需 1&times;D 的注意力） |
| 缓存空间 | O(N &times; D &times; L &times; H)，L=层数，H=头数 |
...
| 生成 N tokens | O(N &times; V) 或 O(N &times; V log V) |
...
| 维度 | 复杂度 |
|------|--------|
| 变量替换 | O(N &times; V)，N=模板长度，V=变量数 |
| 循环渲染 | O(N &times; I)，I=迭代次数 |
| 空间 | O(渲染结果大小) |
...
| 维度 | 复杂度 |
|------|--------|
| 时间 | O(seq² &times; d_model + seq &times; d_model²)，seq² 来自 QK^T |
| 空间 | O(seq² &times; num_heads) 存储注意力权重 |
| 参数量 | 4 &times; d_model² (W_Q, W_K, W_V, W_O) |
...
| 写入 | O(num_tokens &times; num_heads &times; head_dim) |
| 读取 | O(total_tokens &times; num_heads &times; head_dim) |
| 空间 | O(num_blocks &times; block_size &times; num_heads &times; head_dim)，无碎片浪费 |
...
| 维度 | 复杂度 |
|------|--------|
| Faithfulness | O(C &times; L)，C=声明数，L=LLM 调用开销 |
| Relevancy | O(N &times; D)，N=生成问题数，D=Embedding 维度 |
...
| 维度 | 复杂度 |
|------|--------|
| 前向传播 | O(d² + d&times;r&times;2)，与原线性层相比多 O(d&times;r) |
| 可训练参数 | 2&times;d&times;r vs 原始 d²，当 r=8, d=4096 时仅 0.4% |
| 空间 | 额外 2&times;d&times;r 存储 A 和 B |
...
| 维度 | 复杂度 |
|------|--------|
| 时间 | O(S &times; L)，S=步数，L=LLM 调用延迟 |
| 空间 | O(S &times; H)，H=每步历史大小 |
| Token 消耗 | O(S &times; context_length)，随步数线性增长 |
