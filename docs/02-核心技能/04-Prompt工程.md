...
Chain-of-Thought（思维链）是一种让模型**展示推理过程**的技术。类比一下：你让一个人直接算 `123 &times; 456`，他可能算错；但让他一步步写出来，准确率就高很多。CoT 就是让模型「打草稿」。
...
**最佳实践：**
- 图像分辨率不要超过模型支持的上限（通常是 768&times;768 或 1024&times;1024），过大的图会被压缩导致信息丢失
...
- 无缓存：3000 &times; 1000 = 3M tokens/小时
- 有缓存：3000 &times; 1（写入）+ 3000 &times; 999 &times; 0.1 = ~303K tokens/小时
- **节省约 90% 的输入 token 费用**
