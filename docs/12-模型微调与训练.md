...
**类比理解：** 想象一个 1000&times;1000 的大表格（权重矩阵），全量微调要更新所有 100 万个格子。但 LoRA 发现，微调导致的变化其实可以用两个小表格（1000&times;16 和 16&times;1000）相乘来近似表达。存储两个小表格只需要 32000 个数字，远小于 100 万个。
...
2. 在旁边加两个小矩阵 A 和 B
3. 只训练 A 和 B
4. 推理时，把 B&times;A 加回 W：`output = W·x + B·A·x`
...
**类比理解：** 想象一个 1000 人的考试成绩表，1000 科。如果所有科目都可以归结为"文科能力"和"理科能力"两个维度，那么虽然表格很大（1000&times;1000），实际独立变量只有 2 个（秩=2）。这就是低秩——大矩阵的信息可以用更少的维度来表达。

**数学推导：**

原始微调：`W' = W + ΔW`，其中 ΔW 是 d&times;d 的全秩矩阵，有 d² 个参数。
...
- 原始：4096² = 16,777,216
- LoRA：2 &times; 4096 &times; 8 = 65,536
- **减少 99.6%**
...
| 方案 | 硬件 | 时间（1000条） | 成本 |
|------|------|----------------|------|
| 全量微调 7B | 2&times;A100-80GB | 4-8h | $50-100 |
| LoRA 7B | 1&times;A100-80GB | 1-2h | $10-20 |
| QLoRA 7B | 1&times;RTX 4090 | 2-4h | $5-10 |
| API 微调 | - | - | $10-50 |
